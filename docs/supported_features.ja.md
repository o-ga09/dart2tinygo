...
| `wio_terminal`: LCD文字表示、ユーザーLED | 実装済み — `newDisplay()`/`.clear()`/`.drawText()` と `newLed()`/`.on()`/`.off()`/`.toggle()`（`machine.LED`、青）。`tinygo build -target=wioterminal` で確認済み。[`writing_bindings.ja.md`](./writing_bindings.ja.md) の「このリポジトリにあるバインディング」参照 |
