...
- `int`/`double` の算術（`+`/`-`/`*`、単項 `-`、複合代入 `+=`/`-=`/`*=`）は、両辺が一致する型
  ...
  `/`/`/=` は `double` のみ（Dart の `/` は常に `double` を返すため）。上の v0.0.1 の表を参照。
...
型として使う（`Uint8List` は `List<int>` を継承する）。これは v0.0.3 で決めた汎用の
...
fallthrough しないので、case を終わらせるための `break` は生成側で不要（v0.0.1 では
...
v0.0.1 のスコープ外：`case ... when ...` ガード、定数以外／デストラクチャリングの
...
ならない（v0.0.1 には自前のクラス・フィールドがなく、カスケードした getter/setter/
...
節、追加のフィールド・メソッド、値へのコンストラクタ引数はなし（v0.0.1 には
...
#32：`class Foo { ... }` は Go の `struct` + `func NewFoo(...) *Foo` +
...
当面対象外（将来・需要次第）：継承・mixin・インターフェース、ジェネリクス、
値を計算するための initializer list（`: field = expr, ...` — 代わりに通常の
...
| `String` | `string`。`.length` → `len(s)`（バイト長。理由は下の「List<int>」を参照）。`+` 連結、`.substring`。v0.0.1 では一般の添字アクセスなし | 実装済（リテラル／ローカル変数／バインディング戻り値、`+`、`.substring()`、`.length`、`.codeUnits`） |
...
| `List<T>`（`T` が `int` 以外） | `[]T`。`add` → `append`、`length` → `len`、添字はそのまま、`List.filled` → `make` + ループ。growable/fixed は区別しない | 決定（v0.0.3） |
...
| `if` / `else if` / `else` | そのまま対応。各分岐は必ずブロック（上の v0.0.1 の表を参照） | 実装済 |
| `while`（一般条件）/ `for`（宣言変数1つ、updater1つ）/ `break` / `continue` | そのまま対応。上の v0.0.1 の表を参照 | 実装済 |
| `for-in` | → `range` | 決定（v0.0.3） |
