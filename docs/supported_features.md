...
| `wio_terminal`: LCD text, user LED | Implemented — `newDisplay()`/`.clear()`/`.drawText()` and `newLed()`/`.on()`/`.off()`/`.toggle()` (`machine.LED`, blue); verified with `tinygo build -target=wioterminal`. See [`writing_bindings.md`](./writing_bindings.md#reference-bindings-in-this-repository) |
