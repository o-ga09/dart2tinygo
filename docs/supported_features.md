...
| `wio_terminal`: LCD text, user LED, buttons/switch | Implemented — `newDisplay()`/`.clear()`/`.drawText()`, `newLed()`/`.on()`/`.off()`/`.toggle()` (`machine.LED`, blue), and `newButtons()`/`.isPressed(button)`/`.waitPressed(button)` (buttons A/B/C + 5-way switch, pull-up inputs); verified with `tinygo build -target=wioterminal` (`examples/button_led`). See [`writing_bindings.md`](./writing_bindings.md#reference-bindings-in-this-repository) |
