...
| `wio_terminal`: LCD text/shapes/colors, user LED, buttons/switch, buzzer, light/mic ADC, accelerometer | Implemented — `newDisplay()`/`.clear()`/`.drawText()` plus `.width()`/`.height()`/`.fillScreen()`/`.setBacklight()`/`.setRotation()`/`.drawPixel\|Line\|Rect\|Circle()`/`.fillRect\|Circle()`/`.drawTextColor\|Size()`/`.textWidth()` and `rgb(r,g,b)` → `Color`, `newLed()`/`.on()`/`.off()`/`.toggle()` (`machine.LED`, blue), `newButtons()`/`.isPressed(button)`/`.waitPressed(button)` (buttons A/B/C + 5-way switch, pull-up inputs), `newBuzzer()`/`.tone(freqHz)`/`.stop()`/`.beep(freqHz, durationMs)` (`machine.WIO_BUZZER`, TCC0 PWM), `newLightSensor()`/`newMicrophone()` (`machine.WIO_LIGHT`/`WIO_MIC` ADC), and `newAccelerometer()`/`.update()`/`.x()`.../`.xMilliG()`... (LIS3DHTR over I2C1); verified with `tinygo build -target=wioterminal` (`examples/button_led`). See [`writing_bindings.md`](./writing_bindings.md#reference-bindings-in-this-repository) |
