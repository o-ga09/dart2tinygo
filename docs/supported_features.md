...
| `wio_terminal`: LCD text, user LED, buttons/switch, buzzer, light/mic ADC | Implemented — `newDisplay()`/`.clear()`/`.drawText()`, `newLed()`/`.on()`/`.off()`/`.toggle()` (`machine.LED`, blue), `newButtons()`/`.isPressed(button)`/`.waitPressed(button)` (buttons A/B/C + 5-way switch, pull-up inputs), `newBuzzer()`/`.tone(freqHz)`/`.stop()`/`.beep(freqHz, durationMs)` (`machine.WIO_BUZZER`, TCC0 PWM), and `newLightSensor()`/`newMicrophone()` (`machine.WIO_LIGHT`/`WIO_MIC` ADC); verified with `tinygo build -target=wioterminal` (`examples/button_led`). See [`writing_bindings.md`](./writing_bindings.md#reference-bindings-in-this-repository) |
